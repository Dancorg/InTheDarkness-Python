...
p = coso(20, 0, 9, 15, blue,images("assets/tipito.png",13,15),size) #JUGADOR

platforms = []
lights = []
lights_static = []
lights_static.append(dust(716,367,white,0,100,0,0,0, None,True))
...
for i in map_data2:
    ...
    h = i.data['h']/2
    if i.data['color'] == 'red':
        en.append(enem(int(x+w), int(y+h),15,15, red, 4, 1,images("assets/m1515.png",w,h),size))
    if i.data['color'] == 'black':
        en.append(enem(int(x+w), int(y+h),15,15, black, 3, 0,images("assets/m1515.png",w,h),size))
    if i.data['color'] == 'gray':
        en.append(enem(int(x+w), int(y+h),21,21, gray, 3, 1,images("assets/m2121.png",w,h),size))
#en.append(enem(250, 0, 15, 15, red, 4, 1,"m1515.png"))
##en.append(enem(300, 0, 21, 21, gray, 2, 1))
##en.append(enem(350, 0, 27, 27, gray, 3, 1))
...
#                        #pygame.draw.circle(s,(255,255,100,int(10*(mm-dd)/mm)),(xx,yy),qual,0)5
        ...
        i.chispita.append(spark(dat['x'],dat['y']))
...
##        for i in platforms+en+[p]:
##             pygame.draw.rect(s,black,(i.data['x']-30,i.data['y']-1,50,20),0)
...
###############################################################################
    ...
    #map(enem.ojitos(lights,platforms,p,px),en)
##    for i in range(len(obs_map)):
##        for j in range(len(obs_map[i])):
##            if obs_map[i][j][0] == True:
##                pygame.draw.circle(s,(255,0,0,30),(i*10+5,5+j*10),5,0)
    ...
    #s .blit(s, (0,0), special_flags = BLEND_RGBA_MAX)
    #screen.blit(all_sprites, (p.data['x']-p.data['w']/2,p.data['y']-p.data['h']/2))
    screen.blit(s, (0,0))
##    asdf = []
##    for i in lights:
##        asdf.append(i.get_pos_size())

    pygame.display.update()
    #pygame.display.update([p.get_pos_size(),pp,en.get_pos_size()])
